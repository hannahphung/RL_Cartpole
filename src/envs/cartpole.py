...
class CartpoleEnv(gym.Env):
    ...
    def _process_state(self):
        ...
        """
        # No modifications
        processed_state = self.state.copy()
        ...
        #self.previous_state[4], self.previous_state[6], self.previous_state[8], self.previous_state[10]])
        for i in range(len(self.previous_state)):
            if i % 2 == 0:
                processed_state = np.append(processed_state, self.previous_state[i])
    ...
    def reset(self, seed=None, options=None):
        ...
        '''
        # one previous state
        self.previous_state = self.np_random.uniform(low=-0.05, high=0.05, size=(4,))
        self.previous_state = self.previous_state.astype(np.float32)
        '''

        '''
        # 3 previous states
        previous_state1 = self.np_random.uniform(low=-0.05, high=0.05, size=(4,))
        previous_state1 = previous_state1.astype(np.float32)
        previous_state2 = self.np_random.uniform(low=-0.05, high=0.05, size=(4,))
        previous_state2 = previous_state2.astype(np.float32)
        previous_state3 = self.np_random.uniform(low=-0.05, high=0.05, size=(4,))
        previous_state3 = previous_state3.astype(np.float32)

        self.previous_state = np.concatenate((previous_state1,previous_state2,previous_state3))
        '''
        self.previous_state = np.array([])
        # 5 previous state
        for i in range(3):
            previous_state = self.np_random.uniform(low=-0.05, high=0.05, size=(4,))
            previous_state = previous_state.astype(np.float32)
            self.previous_state = np.concatenate((self.previous_state, previous_state))
    ...
    def step(self, action):
        ...
        # ---> TODO: if no accelerations, determine a new working state
        # case of onely keep one previous state
        # self.previous_state = x_tmp

        # keeping multiple previous state
        prev_states = self.previous_state
        # let previous state 1 be the most recent state 
        # that means removing state 3 - oldest state
        # and add temp state to the beginning of the numpy array
        prev_states = np.delete(prev_states, np.s_[8:] )
        self.previous_state = np.concatenate((x_tmp, prev_states))
